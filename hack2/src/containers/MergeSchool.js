...
class MergeSchool extends Component {
    ...
    checkAndUpdateAfterMove = (nextBoard) => {
        if (this.justifyMove(this.state.board, nextBoard.board)) {
            ...
            // console.log(countn)
            // console.log(nextBoard.board)
            // console.log(counts)
            // console.log(this.state.board)
            if (qsRankNow < this.state.best_qs_ranking){
                this.setState({best_qs_ranking: qsRankNow});
            }
            this.setState({board: nextBoardSetWithRandom.board, qs_ranking: qsRankNow, step: stepNow+1});
    ...
    componentDidMount(){
        ...
        body.addEventListener('keydown', this.handleKeyDown, false);
    }
    ...
    render() {
        return (
            <>      
                <Header step={this.state.step} qs_ranking={this.state.qs_ranking} bestqs={this.state.best_qs_ranking} newGame={() => this.initializeBoard()}/>
